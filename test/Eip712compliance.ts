...
import { BuyOrderIssuer } from "../typechain-types";
...
describe("EIP-712 Compliance Test", function() {
    ...
    before(async function() {
        ...
        const orderFees = await ethers.deployContract("OrderFees", [deployer.address, ethers.parseEther("1"), ethers.parseEther("0.005")]);

        // Deploy BuyOrderIssuer contract
        const issuerFactory = await ethers.getContractFactory("BuyOrderIssuer");
        issuerImpl = await ethers.getContractAt("BuyOrderIssuer", await (await upgrades.deployProxy(
            issuerFactory,

...
contract OrderSigUtils {
    ...
    function getOrderRequestHashToSign(IOrderProcessor.Order calldata order, uint256 deadline, uint64 timestamp)
        public
        view
        returns (bytes32)
    {
        ...
        return keccak256(
            abi.encodePacked(
                "\x19\x01",
                orderProcessor.DOMAIN_SEPARATOR(),
                orderProcessor.hashOrderRequest(order, deadline, timestamp)
            )
        );
    }
}
